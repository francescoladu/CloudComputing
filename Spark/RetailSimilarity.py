...
def parse_record(
    ...
    Only buy and fav events are kept.
    Invalid rows and headers are discarded.
    """
    line = line.strip()
    if not line:
        return None
...
def create_user_set(user_id: int) -> Set[int]:
    """
    Creates the first set for a new key.
    """
    return {user_id}
...
def add_user(user_set: Set[int], user_id: int) -> Set[int]:
    """
    Adds a user to the existing set.
    """
...
def merge_user_sets(left: Set[int], right: Set[int]) -> Set[int]:
    """
    Merges two user sets.
 
    The smaller set is merged into the larger one
...
def item_is_usable(
    ...
    """
    Checks if an item/behavior group can generate pairs.
 
    Groups with less than two users are useless.
    Very popular groups can be removed to avoid too many pairs.
    """
    users = record[1]
...
def passes_thresholds(
    ...
    """
    Keeps only pairs that satisfy both minimum thresholds.
    """
...
def delete_output_if_needed(
    ...
    """
    Deletes the output directory if it already exists
    and overwrite mode is enabled.
    """
    java_path = spark_context._jvm.org.apache.hadoop.fs.Path(
        output_path
    )
    ...
    if filesystem.exists(java_path):
        ...
        if not filesystem.delete(java_path, True):
            raise RuntimeError(
                f"Unable to delete output path: {output_path}"
            )
...
def main() -> None:
    ...
    try:
        ...
        # Parses the rows and removes invalid records.
        parsed = (
            lines
            .map(
